...
import com.webi.webiMoney.config.JwtTokenUtil;
import com.webi.webiMoney.model.JwtRequest;
import com.webi.webiMoney.model.JwtResponse;
import com.webi.webiMoney.model.User;
import com.webi.webiMoney.repository.UserRepository;
import com.webi.webiMoney.services.UserDetailsServiceImpl;
...
public class JwtAuthenticationController {
    ...
    public @ResponseBody String createLoginToken(@RequestBody JwtRequest authenticationRequest) throws Exception {
        User user=userRepository.userByUsername(authenticationRequest.getUsername());
        if(user.getStatus().equalsIgnoreCase("bloquer")){
            return "Vous etes bloquer veuillez-contacter l'administrateur";
        }
        authenticate(authenticationRequest.getUsername(), authenticationRequest.getPassword());
        final UserDetails userDetails = userDetailsService
                .loadUserByUsername(authenticationRequest.getUsername());
    ...
    private void authenticate(String username, String password) throws Exception {
        try {
            ...
            throw new Exception("Login ou mot de pass incorrect", e);
        }
    }
